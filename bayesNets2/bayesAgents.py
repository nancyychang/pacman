...
def constructBayesNet(gameState):
    ...
    edgeTuplesList = [(X_POS_VAR, FOOD_HOUSE_VAR), (X_POS_VAR, GHOST_HOUSE_VAR), (Y_POS_VAR, FOOD_HOUSE_VAR), (Y_POS_VAR, GHOST_HOUSE_VAR)]

    for obs in obsVars:
        edgeTuplesList.append((FOOD_HOUSE_VAR, obs))
        edgeTuplesList.append((GHOST_HOUSE_VAR, obs))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    maxProb = -float("inf")
    mostLikelyFoodHousePosition = dict()

    for assignment in factor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment)
        if prob > maxProb:
            maxProb = prob
            mostLikelyFoodHousePosition = assignment
    return mostLikelyFoodHousePosition
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        boardTopLeft, boardTopRight = {}, {}
        boardTopLeft.update(evidence)
        boardTopRight.update(evidence)

        boardTopLeft[FOOD_HOUSE_VAR], boardTopRight[FOOD_HOUSE_VAR] = TOP_LEFT_VAL, TOP_RIGHT_VAL
        boardTopLeft[GHOST_HOUSE_VAR], boardTopRight[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL, TOP_LEFT_VAL

        leftExpectedValue = (factor.getProbability(boardTopLeft)*WON_GAME_REWARD) + (factor.getProbability(boardTopRight)*GHOST_COLLISION_REWARD)
        rightExpectedValue = (factor.getProbability(boardTopLeft)*GHOST_COLLISION_REWARD) + (factor.getProbability(boardTopRight)*WON_GAME_REWARD)
