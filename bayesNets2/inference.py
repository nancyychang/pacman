...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        listOfFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variableToJoin in eliminationOrder:
            listOfFactors, factorToJoin = joinFactorsByVariable(listOfFactors, variableToJoin)
            
            if len(factorToJoin.unconditionedVariables()) > 1:
                factorToElim = eliminate(factorToJoin, variableToJoin)
                listOfFactors.append(factorToElim)
        return normalize(joinFactors(listOfFactors))
