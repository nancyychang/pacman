...
def enhancedFeatureExtractor(datum):
    ...
    while allCoordinates:
        firstcoord = allCoordinates.pop(0)
        if datum[firstcoord[0]][firstcoord[1]] == 0:
            queue.push(firstcoord)
            numWhiteSpaces += 1
        while queue.isEmpty() == False:
            coordinates = queue.pop()
            if datum[coordinates[0]][coordinates[1]] == 0:
                if coordinates[0] + 1 < 28:
                    if (coordinates[0] + 1, coordinates[1]) in allCoordinates:
                        queue.push((coordinates[0] + 1, coordinates[1]))
                        allCoordinates.remove((coordinates[0] + 1, coordinates[1]))
                if coordinates[1] + 1 < 28:
                    if (coordinates[0], coordinates[1] + 1) in allCoordinates:
                        queue.push((coordinates[0], coordinates[1] + 1))
                        allCoordinates.remove((coordinates[0], coordinates[1] + 1))
