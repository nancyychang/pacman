...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                "*** YOUR CODE HERE ***"
                train_label, train_input = label_train_data[i], self.classify([input_train_data[i]])[0]
                if train_label != train_input:
                    self.weights[train_label] += input_train_data[i]
                    self.weights[train_input] -= input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        for weight in list(self.weights[label]):
            if len(best100Features) == 100:
                break
            best100Features.append(max(self.weights[label]))
            del self.weights[label][max(self.weights[label])]
