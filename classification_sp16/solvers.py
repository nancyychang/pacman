...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            new_param_tensor = tf.sub(param_vars[i], tf.mul(self.learning_rate, grad_tensors[i]))
            updates.append((param_vars[i], new_param_tensor))
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(vel_vars)):
            new_vel_var = tf.sub(tf.mul(self.momentum, vel_vars[i]), tf.mul(self.learning_rate, grad_tensors[i]))
            updates.append((vel_vars[i], new_vel_var))
        for i in range(len(param_vars)):
            new_param_tensor = param_vars[i] + updates[i][1]
            updates.append((param_vars[i], new_param_tensor))
        return updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss,updates = session.run([loss_tensor, update_ops], feed_dict={model.input_ph: input_train_data, target_ph: target_train_data})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_gen = tfu.MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        val_gen = tfu.MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            inp_train, tar_train = train_gen.next()
            inp_val, tar_val = val_gen.next()

            train_loss,updates = session.run([loss_tensor, update_ops], feed_dict={model.input_ph: inp_train, target_ph: tar_train})
            val_loss = session.run(loss_tensor, feed_dict={model.input_ph: inp_val, target_ph: tar_val})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_gen = tfu.MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
        val_gen = tfu.MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            inp_train, tar_train = train_gen.next()
            inp_val, tar_val = val_gen.next()
            train_loss = session.run(loss_tensor, feed_dict={model.input_ph: inp_train, target_ph: tar_train})
            val_loss = session.run(loss_tensor, feed_dict={model.input_ph: inp_val, target_ph: tar_val})
