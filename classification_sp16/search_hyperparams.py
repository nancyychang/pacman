...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        solver = solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum=momentum)
    ...
    best_model = models[val_accuracies.index(highest_accuracy)]
    best_hyperparams = hyperparams[val_accuracies.index(highest_accuracy)]
